...
<?php if ( ! have_posts() ) : ?>
		<h1><?php _e( 'Ups...', 'twentyten' ); ?></h1>
		<p><?php _e( 'Leider ist die Seite nicht (mehr) verfügbar.', 'twentyten' ); ?></p>
		<?php get_search_form(); ?>
...
<?php the_excerpt(); ?>
<?php else : ?>
<?php the_content( __( 'Weiterlesen <i class="fa fa-angle-double-right"></i>', 'twentyten' ) ); ?>
<?php endif; ?>
<?php twentyten_posted_on(); ?>
|
<?php comments_popup_link( __( 'Leave a comment', 'twentyten' ), __( '1 Comment', 'twentyten' ), __( '% Comments', 'twentyten' ) ); ?>
...
<?php /* UNSERE BEITRÄGE AB HIER ###################################################### */ ?>

	<?php else : ?>
			<section class="dennis-post-box">
			<h2><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'twentyten' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
			<?php // twentyten_posted_on(); ?>
...
<?php if (  $wp_query->max_num_pages > 1 ) : ?>
				...
				<?php previous_posts_link( __( 'Neuere Beiträge <i class="fa fa-angle-double-right"></i>', 'twentyten' ) ); ?>
<?php endif; ?>